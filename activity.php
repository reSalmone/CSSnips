...
<body>
    ...
    <div id="rest" onclick="closeLogin(); closeSignup();">
        ...
        if ($username != '' && $type != '') {
            ...
            <form action="activity.php" method="GET" class="search-form" id="search-form">
            ...
            <div class="search-output-div">
                ...
                if ($dbcon != -1) { //se la connessione è correttamente stabilita
                    ...
                    if ($search != '') {
                        ...
                        if ($type == 'activity') {
                            $q1 = $q1Base . " SELECT * FROM filtered_snips WHERE creator = '$username' ORDER BY relevance DESC, likes DESC, views DESC LIMIT $2 OFFSET $3;";
                            $q1All = $q1Base . " SELECT COUNT(*) FROM filtered_snips WHERE creator = '$username';";
                        } else if ($type == 'watchlist') {
                            $q1 = $q1Base . " SELECT * FROM filtered_snips fs
                            JOIN (SELECT unnest(savedsnippets) AS name FROM users WHERE username = '$username') watchlist ON fs.file_location = watchlist.name ORDER BY relevance DESC, likes DESC, views DESC LIMIT $2 OFFSET $3;";
                            $q1All = $q1Base . " SELECT COUNT(*) FROM filtered_snips fs
                            JOIN (SELECT unnest(savedsnippets) AS name FROM users WHERE username = '$username') watchlist ON fs.file_location = watchlist.name;";
                        } else if ($type == 'liked') {
                            $q1 = $q1Base . " SELECT * FROM filtered_snips fs JOIN (SELECT unnest(likedsnippets) AS name FROM users WHERE username = '$username') liked ON fs.file_location = liked.name ORDER BY relevance DESC, likes DESC, views DESC LIMIT $2 OFFSET $3;";
                            $q1All = $q1Base . " SELECT COUNT(*) FROM filtered_snips fs JOIN (SELECT unnest(likedsnippets) AS name FROM users WHERE username = '$username') liked ON fs.file_location = liked.name;";
                        }
                        $resultPage = pg_query_params($dbcon, $q1, array($search, $pageSize, $pageOffset));
                        $resultAll = pg_query_params($dbcon, $q1All, array($search));
                    } else {
                        if ($type == 'activity') {
                            $q1 = "SELECT * FROM snips WHERE creator = '$username' ORDER BY likes DESC, views DESC LIMIT $1 OFFSET $2;"; //se non viene effettuata una ricerca
                            $q1All = "SELECT COUNT(*) FROM snips WHERE creator = '$username';";
                        } else if ($type == 'watchlist') {
                            $q1 = "with abc as(SELECT unnest(savedsnippets) AS names FROM users WHERE username = '$username') SELECT * FROM snips x join abc y on x.file_location = y.names ORDER BY likes DESC, views DESC LIMIT $1 OFFSET $2;"; //se non viene effettuata una ricerca
                            $q1All = "with abc as(SELECT unnest(savedsnippets) AS names FROM users WHERE username = '$username') SELECT count(*) FROM snips x join abc y on x.file_location = y.names;";
                        } else if ($type == 'liked') {
                            $q1 = "with abc as(SELECT unnest(likedsnippets) AS names FROM users WHERE username = '$username') 
                            SELECT * FROM snips x join abc y on x.file_location = y.names ORDER BY likes DESC, views DESC LIMIT $1 OFFSET $2;"; //se non viene effettuata una ricerca   
                            $q1All = "with abc as(SELECT unnest(likedsnippets) AS names FROM users WHERE username = '$username') SELECT count(*) FROM snips x join abc y on x.file_location = y.names;";
                        }
            ...
            <main class="profilo-container">
                <var class="main_var">
                    <section class="profilo-info">
                        <h2>"MISSING USERNAME OR TYPE."</h2>
                    </section>
                </var>
            </main>
            <?php
